...
#         num_think_tokens: int = 250,
#         min_think_monitor_tokens: int = 1,
...
#             # 检查是否已经生成 </think>，如果有，则强制输出 "\n\n<action>"
#             if self.think_end_token in ids_b:
#                 if self.follow_think_end_token:
...
#             # (1) 长度限制，只能发生一次
#             if tokens_since_think >= self.num_think_tokens and not getattr(self, '_think_length_limited', [False]*batch_size)[b]:
#                 if not hasattr(self, '_think_length_limited'):
#                     self._think_length_limited = [False] * batch_size
#                 if safe_force_token(logits[b], self.think_end_token):
#                     self._force_queue[b] = list(self.follow_think_end_token)
#                     self._think_length_limited[b] = True
#                 self._last_score[b] = None
#                 self._stagnation_count[b] = 0
#                 continue
