...
from transformers import AutoConfig, AutoModelForCausalLM, PreTrainedModel, AutoModelForImageTextToText
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                if position_ids.dim() == 3:
                    from verl.utils.ulysses import ulysses_pad, slice_input_tensor
                    input_ids_rmpad, position_ids_rmpad, pad_size = ulysses_pad(
                        input_ids_rmpad, position_ids_rmpad, sp_size=get_ulysses_sequence_parallel_world_size()
                    )
                    assert input_ids_rmpad.shape[1] == position_ids_rmpad.shape[2], (
                        f"After padding, input_ids seqlen ({input_ids_rmpad.shape[1]}) != "
                        f"position_ids seqlen ({position_ids_rmpad.shape[2]})"
                    )
                    input_ids_rmpad_sliced = slice_input_tensor(input_ids_rmpad, dim=1, padding=False)
                    actual_seqlen = input_ids_rmpad_sliced.shape[1]
                    
                    position_ids_rmpad_padded = slice_input_tensor(position_ids_rmpad, dim=2, padding=False)
                    position_ids_rmpad_padded = position_ids_rmpad_padded.squeeze(1)
                    
                    if position_ids_rmpad_padded.shape[1] != actual_seqlen:
                        if position_ids_rmpad_padded.shape[1] > actual_seqlen:
                            position_ids_rmpad_padded = position_ids_rmpad_padded[:, :actual_seqlen]
                        else:
                            last_pos = position_ids_rmpad_padded[:, -1:]
                            pad_size = actual_seqlen - position_ids_rmpad_padded.shape[1]
                            position_ids_rmpad_padded = torch.cat([
                                position_ids_rmpad_padded,
                                last_pos.expand(-1, pad_size)
                            ], dim=1)
                    
                    position_ids_rmpad_padded = torch.clamp(position_ids_rmpad_padded, min=0, max=actual_seqlen - 1)
                    
                    assert input_ids_rmpad_sliced.shape[1] == position_ids_rmpad_padded.shape[1], (
                        f"input_ids seqlen ({input_ids_rmpad_sliced.shape[1]}) != "
                        f"position_ids seqlen ({position_ids_rmpad_padded.shape[1]})"
                    )
                    max_pos_id = position_ids_rmpad_padded.max().item()
                    assert max_pos_id < actual_seqlen, (
                        f"position_ids max value ({max_pos_id}) >= actual_seqlen ({actual_seqlen})"
                    )
                else:
                ...
                if position_ids.dim() == 3:
                    ...
                    position_ids_rmpad_padded = torch.clamp(
                        position_ids_rmpad_padded, 
                        min=0, 
                        max=actual_seqlen - 1
                    )
                    assert position_ids_rmpad_padded.shape[1] == actual_seqlen, (
                        f"position_ids seqlen ({position_ids_rmpad_padded.shape[1]}) != "
                        f"input_ids seqlen ({actual_seqlen})"
                    )
                    max_pos_id = position_ids_rmpad_padded.max().item()
                    assert max_pos_id < actual_seqlen, (
                        f"position_ids max value ({max_pos_id}) >= actual_seqlen ({actual_seqlen})"
...
def run_sft(config):
    ...
    processor = hf_processor(local_model_path, trust_remote_code=config.model.trust_remote_code, use_fast=True)  # used for multimodal LLM, could be none
