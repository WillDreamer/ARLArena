...
os.environ["ROOT_PATH"] = project_root
...
def main(config):
    ...
    for test_data in val_dataloader:
        ...
        # Note: multi_modal_data should NOT be popped here - it needs to be preserved
        # for multimodal processing in the rollout loop
        ...
        # Extract task scores - for game agents, use 'success_rate' as the main metric
        # Also check for environment-specific metrics if available
        task_scores = result['success'].get('success_rate', None)
        if task_scores is None:
            # Fallback to other available success metrics
