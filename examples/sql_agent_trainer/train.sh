set -x
dataset_name=skysql
train_data=$(pwd)/datasets/${dataset_name}/train.parquet
val_data=[$(pwd)/datasets/${dataset_name}/test_spider_dev.parquet,\
$(pwd)/datasets/${dataset_name}/test_spider_dk.parquet,\
$(pwd)/datasets/${dataset_name}/test_spider_test.parquet,\
$(pwd)/datasets/${dataset_name}/test_spider_syn.parquet,\
$(pwd)/datasets/${dataset_name}/test_spider_realistic.parquet]
...
python -m recipe.sql_agent.servers.serve --host $host --port $port --tool_type "sql" --workers_per_tool 4 &
# --done-if-invalid True
server_pid=$!

echo "Server (pid=$server_pid) started at $tool_server_url"

PYTHONUNBUFFERED=1 python3 -m recipe.sql_agent.trainer.main_ppo \
